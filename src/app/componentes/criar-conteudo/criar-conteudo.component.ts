  ...
  criarConteudo() {
    ...
    if (this.id != null) {
      this._conteudoService.putConteudo(this.id, CONTEUDO).subscribe(data => {
        this.toastr.success('O conteúdo foi editado com êxito', 'Conteúdo atualizado!', {positionClass: 'toast-bottom-right'});
      ...
      this._conteudoService.postConteudo(CONTEUDO).subscribe(data => {
        this.toastr.success('O conteúdo foi criado com êxito', 'Conteúdo adicionado!', {positionClass: 'toast-bottom-right'});
        this.router.navigate(['/listar-conteudo']);
      }, err => {
